...
public partial class ThemeViewModel : ObservableRecipient
{
    ...
    private Brush BrushConvter(
            string colorTxt,
            string defaultColor = "0x000000",
            string colorFormat = "abgr"
        )
    {
        ...
        try
        {
            ...
            switch (colorFormat)
            {
                ...
                case "rgba":
                    colorStr =
                        "#"
                        + $"{_cArray[6]}{_cArray[7]}{_cArray[0]}{_cArray[1]}{_cArray[2]}{_cArray[3]}{_cArray[4]}{_cArray[5]}";
                    targetColor = (Color)ColorConverter.ConvertFromString(colorStr);
                    break;
                ...
                default:
                    ...
                    colorStr =
                        "#"
                        + $"{_cArray[0]}{_cArray[1]}{_cArray[6]}{_cArray[7]}{_cArray[4]}{_cArray[5]}{_cArray[2]}{_cArray[3]}";
                    targetColor = (Color)ColorConverter.ConvertFromString(colorStr);
                    break;
            }
    ...
    private void LoadCurrentSkin()
    {
        ...
        try
        {
            ...
            ConfigHelper.WriteConfigByString(
                "color_scheme",
                WeaselCustomDetails.patch.style.color_scheme
            );
        ...
        catch (Exception ex)
        {
            ...
            var default_skin = ColorsList.FirstOrDefault(c =>
                bool.Parse(c.description.is_template) == true
            );
    ...
    private void LoadCustomColor()
    {
        ...
        try
        {
            ...
            ConfigModel.ColorIndex = ColorsList
                .Select(c => c.description.color_name)
                .ToList()
                .IndexOf(shemeName);
        }
        catch (Exception ex)
    ...
    private void ReLoadColorShemes()
    {
        ...
        ConfigModel.ColorIndex = ColorsList
            .Select(c => c.description.color_name)
            .ToList()
            .IndexOf(CurrentSkin.Style.color_scheme);
    }
    ...
    private void SaveDarkSchema()
    {
        ...
        var has_dark_skin = DarkColorsList.FirstOrDefault(d =>
            d.style.color_scheme == dark_skin_name
        );
        ...
        {
            var dark_skin = ColorsList
                .Where(c => c.style.color_scheme == dark_skin_name)
                .FirstOrDefault();
            if (dark_skin == null)
            {
                throw new Exception($"找不到夜间皮肤信息：{dark_skin_name}");
            }
            ...
            if (!CurrentSkin.PresetColorSchemes.ContainsKey(dark_skin_name))
                CurrentSkin.PresetColorSchemes.Add(
                    dark_skin_name,
                    dark_skin_dict.FirstOrDefault().Value
                );
        }
    ...
    public void AIColor()
    {
        ...
        sourceThemes.Source = new Uri(
            "pack://application:,,,/WubiMaster;component/Themes/ThemeNames.xaml"
        );
        ...
        theme_resource.Source = new Uri(
            $"pack://application:,,,/WubiMaster;component/Themes/{usedTheme.Value}.xaml"
        );
        ...
        App.Current.Dispatcher.BeginInvoke(() =>
        {
            try
            {
                ...
                SolidColorBrush hilited_candidate_text_color = (SolidColorBrush)
                    theme_resource["bg-100"];
                SolidColorBrush hilited_candidate_back_color = (SolidColorBrush)
                    theme_resource["primary-100"];
                SolidColorBrush hilited_label_color = (SolidColorBrush)theme_resource["bg-200"];
                SolidColorBrush hilited_comment_text_color = (SolidColorBrush)
                    theme_resource["bg-200"];
                 ...
                 *    其他文字色：默认
                 */
                List<string> primary_colors = new List<string>()
                {
                    "primary-100",
                    "primary-200",
                    "primary-300"
                ...
                if (hilited_candidate_index == 0)
                {
                    ...
                    border_color = (SolidColorBrush)
                        theme_resource[bor_list[new Random().Next(bor_list.Count)]];
                    // 首选背景色
                    hilited_candidate_back_color = (SolidColorBrush)
                        theme_resource[primary_colors[new Random().Next(primary_colors.Count)]];
                    // 首选文字色
                    ...
                    hilited_text_color = (SolidColorBrush)
                        theme_resource[accent_colors[accent_incex]];
                    accent_colors.RemoveAt(accent_incex);
                    hilited_back_color = (SolidColorBrush)
                        theme_resource[accent_colors[new Random().Next(accent_colors.Count)]];
                }
                else if (hilited_candidate_index == 1)
                {
                    ...
                    hilited_candidate_back_color = (SolidColorBrush)
                        theme_resource[accent_colors[accent_index]];
                    // 首选文字色
                    accent_colors.RemoveAt(accent_index);
                    hilited_candidate_text_color = (SolidColorBrush)
                        theme_resource[accent_colors[new Random().Next(accent_colors.Count)]];
                    hilited_label_color = hilited_candidate_text_color;
                    hilited_comment_text_color = hilited_candidate_text_color;
                    // 编码区颜色
                ...
                else if (hilited_candidate_index == 2)
                {
                    ...
                    // 背景色
                    back_color = (SolidColorBrush)
                        theme_resource[bg_list[new Random().Next(bg_list.Count)]];
                    border_color = (SolidColorBrush)
                        theme_resource[bg_list[new Random().Next(bg_list.Count)]];
                    // 首选背景色
                    hilited_candidate_back_color = (SolidColorBrush)
                        theme_resource[bg_colors[new Random().Next(bg_colors.Count)]];
                    // 首选文字色
                ...
                else if (hilited_candidate_index == 3)
                {
                    ...
                    int accent_index = new Random().Next(accent_colors.Count);
                    hilited_label_color = (SolidColorBrush)
                        theme_resource[accent_colors[accent_index]];
                    accent_colors.RemoveAt(accent_index);
                    hilited_comment_text_color = (SolidColorBrush)
                        theme_resource[accent_colors[new Random().Next(accent_colors.Count)]];
                    // 编码区颜色
                ...
                CurrentSkin.UsedColor.text_color = ColorConverterHelper.ConverterToRime(
                    text_color.ToString()
                );
                CurrentSkin.UsedColor.comment_text_color = ColorConverterHelper.ConverterToRime(
                    comment_text_color.ToString()
                );
                CurrentSkin.UsedColor.label_color = ColorConverterHelper.ConverterToRime(
                    label_color.ToString()
                );
                CurrentSkin.UsedColor.back_color = ColorConverterHelper.ConverterToRime(
                    back_color.ToString()
                );
                CurrentSkin.UsedColor.shadow_color = ColorConverterHelper.ConverterToRime(
                    shadow_color.ToString()
                );
                CurrentSkin.UsedColor.border_color = ColorConverterHelper.ConverterToRime(
                    border_color.ToString()
                );
                CurrentSkin.UsedColor.hilited_text_color = ColorConverterHelper.ConverterToRime(
                    hilited_text_color.ToString()
                );
                CurrentSkin.UsedColor.hilited_back_color = ColorConverterHelper.ConverterToRime(
                    hilited_back_color.ToString()
                );
                CurrentSkin.UsedColor.hilited_candidate_text_color =
                    ColorConverterHelper.ConverterToRime(hilited_candidate_text_color.ToString());
                CurrentSkin.UsedColor.hilited_candidate_back_color =
                    ColorConverterHelper.ConverterToRime(hilited_candidate_back_color.ToString());
                CurrentSkin.UsedColor.hilited_label_color = ColorConverterHelper.ConverterToRime(
                    hilited_label_color.ToString()
                );
                CurrentSkin.UsedColor.hilited_comment_text_color =
                    ColorConverterHelper.ConverterToRime(hilited_comment_text_color.ToString());
                CurrentSkin.UsedColor.candidate_text_color = ColorConverterHelper.ConverterToRime(
                    candidate_text_color.ToString()
                );
                CurrentSkin.UsedColor.candidate_back_color = ColorConverterHelper.ConverterToRime(
                    candidate_back_color.ToString()
                );
    ...
    public void ChangeSkin(object obj)
    {
        ...
        try
        {
            ...
            if (cModel == null)
                throw new NullReferenceException($"找不到皮肤对象: {obj.ToString()}");
            ...
            _colorModel.UsedColor = CopyOut.TransReflection<ColorScheme, ColorScheme>(
                cModel.preset_color_schemes.FirstOrDefault().Value
            );
            foreach (var k in cModel.preset_color_schemes.Keys)
            {
                var _schema = CopyOut.TransReflection<ColorScheme, ColorScheme>(
                    cModel.preset_color_schemes[k]
                );
        ...
        App.Current.Dispatcher.BeginInvoke(() =>
        {
            try
            {
                ...
                if (ConfigModel.Theme_FollowTheme)
                {
                    ...
                    SolidColorBrush text_color = (SolidColorBrush)
                        App.Current.FindResource("text-100");
                    SolidColorBrush comment_text_color = (SolidColorBrush)
                        App.Current.FindResource("text-200");
                    SolidColorBrush label_color = (SolidColorBrush)
                        App.Current.FindResource("text-200");
                    SolidColorBrush back_color = (SolidColorBrush)
                        App.Current.FindResource("bg-100");
                    SolidColorBrush shadow_color = (SolidColorBrush)
                        App.Current.FindResource("primary-300");
                    SolidColorBrush border_color = (SolidColorBrush)
                        App.Current.FindResource("primary-200");
                    SolidColorBrush hilited_text_color = (SolidColorBrush)
                        App.Current.FindResource("accent-200");
                    SolidColorBrush hilited_back_color = (SolidColorBrush)
                        App.Current.FindResource("accent-100");
                    SolidColorBrush hilited_candidate_text_color = (SolidColorBrush)
                        App.Current.FindResource("bg-100");
                    SolidColorBrush hilited_candidate_back_color = (SolidColorBrush)
                        App.Current.FindResource("primary-100");
                    SolidColorBrush hilited_label_color = (SolidColorBrush)
                        App.Current.FindResource("bg-200");
                    SolidColorBrush hilited_comment_text_color = (SolidColorBrush)
                        App.Current.FindResource("bg-200");
                    SolidColorBrush candidate_text_color = (SolidColorBrush)
                        App.Current.FindResource("text-100");
                    SolidColorBrush candidate_back_color = (SolidColorBrush)
                        App.Current.FindResource("bg-100");
                    ...
                    CurrentSkin.UsedColor.hilited_candidate_text_color =
                        ColorConverterHelper.ConverterToRime(
                            hilited_candidate_text_color.ToString()
                        );
                    CurrentSkin.UsedColor.hilited_candidate_back_color =
                        ColorConverterHelper.ConverterToRime(
                            hilited_candidate_back_color.ToString()
                        );
                    CurrentSkin.UsedColor.hilited_label_color =
                        ColorConverterHelper.ConverterToRime(hilited_label_color.ToString());
                    CurrentSkin.UsedColor.hilited_comment_text_color =
                        ColorConverterHelper.ConverterToRime(hilited_comment_text_color.ToString());
                    CurrentSkin.UsedColor.candidate_text_color =
                        ColorConverterHelper.ConverterToRime(candidate_text_color.ToString());
                    CurrentSkin.UsedColor.candidate_back_color =
                        ColorConverterHelper.ConverterToRime(candidate_back_color.ToString());
    ...
    public void CreateNewSkin(object obj)
    {
        ...
        {
            var new_skin_count =
                ColorsList.Where(c => c.description.color_name.Contains("Skin-")).Count() + 1;
            ...
            csModel.PresetColorSchemes.Add(
                new_skin_name,
                default_color.preset_color_schemes.FirstOrDefault().Value
            );
        ...
        if (openFileDialog.ShowDialog() == true)
        {
            ...
            if (file_ext.ToLower() == ".yaml")
            {
                try
                {
                    ...
                    if (
                        ColorsList.Any(c =>
                            c.description.color_name == cModel.description.color_name
                        )
                    )
                    {
    ...
    public void SetSkinColor(object obj)
    {
        ...
        switch (color_name)
        {
            ...
            case "hilited_candidate_back_color":
                CurrentSkin.UsedColor.hilited_candidate_back_color =
                    ColorConverterHelper.ConverterToRime(cpv.CurrentBrush.ToString());
                break;

            case "hilited_candidate_border_color":
                CurrentSkin.UsedColor.hilited_candidate_border_color =
                    ColorConverterHelper.ConverterToRime(cpv.CurrentBrush.ToString());
                break;
